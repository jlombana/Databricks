...
# MAGIC %md # Feature Store taxi example - create input dataset
# MAGIC
# MAGIC This notebook creates the input dataset used by the Feature Store taxi example notebook.
# MAGIC
# MAGIC It includes these steps to preprocess the data:
# MAGIC
# MAGIC 1. Creates the `feature_store_demo` database.
# MAGIC 1. Reads in a subset of the [NYC Taxi Data](https://www1.nyc.gov/site/tlc/about/tlc-trip-record-data.page) from `dbfs:/databricks-datasets/nyctaxi`.
# MAGIC 1. Renames certain columns and drops unnecessary columns.
# MAGIC 1. Applies a UDF to convert latitude and longitude coordinates into ZIP codes and adds a ZIP code column to the DataFrame. This conversion is based on 
# MAGIC [Extracting ZIP codes from longitude and latitude in PySpark](https://medium.com/@m.majidpour/extracting-zip-codes-from-longitude-and-latitude-in-pyspark-eafbcfef474c).
# MAGIC 1. Writes the result to a new table: `feature_store_taxi_example` in the `nyc_yellow_taxi_with_zips` database.
# MAGIC
# MAGIC ## Requirements
...
# MAGIC %md ## Setup and installation
# MAGIC
# MAGIC To convert the latitude and longitude data in the original dataset into ZIP codes, this notebook uses the Python package `uszipcode`, [available from PyPI](https://pypi.org/project/uszipcode/).
